...
export class TelegramBridge {
    ...
    async onAssistantText(chat, text, richHtml) {
        ...
        if (richHtml !== undefined) {
            try {
                ...
                if (!(error instanceof TelegramApiError) || (error.code !== 400 && error.code !== 404))
                    throw error;
            }
        }
        const delivered = await this.deliver(chat.chatId, text);
        if (delivered.length > 0)
            chat.latestAssistantMessageIds = delivered;
    }
    /**
    ...
    async onTurnEnd(chat, event, notice) {
        ...
        if (notice !== undefined)
            await this.safeSend(chat.chatId, notice);
    }
