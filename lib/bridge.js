...
const TELEGRAM_CHANNEL_PROMPT = [
    ...
    "- <tg-thinking> 仅用于生成中的草稿，由插件负责插入状态；不要在最终答案中输出这个标签。插件会将 dsh 已提供的中间消息放入外层折叠记录，标题显示工具调用次数和中间消息数量，不包含思考或工具详情；无需在正文重复过程记录。",
...
export class TelegramBridge {
    ...
    async handleUpdate(update) {
        ...
        if (!hasImage && message.document === undefined && message.text === undefined && message.reply_to_message === undefined)
            return;
        ...
        switch (command) {
            ...
            case '/resend':
                try {
                    ...
                    if (!sent)
                        await this.safeSend(chatId, 'ℹ️ **没有可重发的结果**\n\n当前还没有缓存的完整结果。');
                }
                catch (error) {
            ...
            case '/collect': {
                ...
                await this.sendCollectionStatus(collection, `📥 **已进入收集模式**\n\n现在可以按任意顺序发送文字、图片和文件。\n\n---\n\n${COLLECTION_ACTIONS}`);
                break;
            }
            case '/send': {
                const active = this.chats.get(String(chatId))?.active;
            ...
            case '/discard': {
                ...
                await this.sendCollectionStatus(collection, '🗑 **已放弃本次收集**\n\n已经下载的图片仍保留在当前工作区的 `telegram-downloads` 文件夹中。');
                break;
            }
            case '/followup':
                await this.safeSend(chatId, FOLLOWUP_HELP);
                break;
            case '/help':
        ...
        try {
            ...
            if (view === 'home') {
                ...
                rows.push([button('📦 归档会话', async () => {
                        const archived = await this.archiveCurrent(chatId);
                        await this.showMenu(chatId, 'home', 0, undefined, archived === undefined ? '没有可归档的会话。' : '已归档会话，聊天记录已保留；可新建会话或在会话列表中查看。', messageId);
                    })]);
            }
    ...
    async sendCollectionStatus(collection, text) {
    ...
    async addToCollection(active, collection, message, overrideText, announce = true) {
        try {
            ...
            if (announce)
                await this.sendCollectionStatus(collection, this.collectionStatus(collection));
            return true;
        }
        catch (error) {
    ...
    async submitCollection(active, collection, mode) {
        ...
        try {
            ...
            await this.sendCollectionStatus(collection, mode === 'followup' ? '🕒 **已加入后续任务队列**' : '✅ **已提交收集内容**');
        }
        catch (error) {
    ...
    async telegramFilePart(message) {
        const document = message.document;
        if (document === undefined || this.imageDocument(message))
            return undefined;
        const downloaded = await this.client.downloadFile(document.file_id, 20 * 1024 * 1024, this.abortController.signal);
    ...
    attachTelegramBinding(chatId, agent) {
        ...
        try {
            ...
            disposeApprovals = agent.ctx.on('approval/request', (request, next) => {
                const active = this.chats.get(String(chatId))?.active;
                if (active?.agent !== agent || request.agent !== agent)
                    return next();
    ...
    async approveThroughTelegram(chatId, agent, request) {
        try {
    ...
    askThroughTelegram(chatId, agent, request, approval = false) {
    ...
    handleSessionEvent(session, event) {
        ...
        switch (event.type) {
            case 'turn/start':
                ...
                chat.progress = new TelegramProgress(this.client, chat.chatId, this.abortController.signal, task => this.enqueue(chat, task), error => this.ctx.logger.warn('[telegram] preview failed: %s', messageOf(error)));
                break;
