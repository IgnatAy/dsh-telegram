        ...
        switch (event.type) {
            case 'turn/start':
                ...
                if (this.client.sendMessageDraft !== undefined || this.client.sendRichMessageDraft !== undefined) {
                    chat.progress = new TelegramProgress(this.client, chat.chatId, this.abortController.signal, task => this.enqueue(chat, task), id => chat.transientMessageIds.add(id), error => this.ctx.logger.warn('[telegram] preview failed: %s', messageOf(error)), this.maxMessageLength);
                }
                break;
