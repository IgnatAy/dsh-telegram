...
export class TelegramBridge {
    ...
    async showMenu(chatId, view = 'home', page = 0, workspaceId, notice) {
        try {
            const state = this.stateFor(chatId);
            ...
            const navigate = (next, nextPage = 0, id) => this.showMenu(chatId, next, nextPage, id);
            const lines = [
                '# 🎛 控制面板',
