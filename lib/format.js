        ...
        if (marker && !/\s/.test(text[i + marker.length] ?? ' ')
            ...
            for (; end < text.length; end++) {
                ...
                if (text[end] === '`') {
                    ...
                    if (codeSpan) {
                        end += codeSpan[0].length - 1;
                        continue;
                    }
                }
                if (marker.length === 2 && marker !== '~~' && text.startsWith(marker + marker[0], end))
                    end++;
                if (text.startsWith(marker, end) && !/\s/.test(text[end - 1])
                    && !(marker[0] === '_' && /[\p{L}\p{N}]/u.test(text[end + marker.length] ?? '')))
                    break;
            }
            if (end < text.length && !text.slice(i, end).includes('\n')) {
                ...
                for (const part of inlineRuns(text.slice(i + marker.length, end), bold, allowLinks)) {
                    ...
                    result.push(part.open === '<code>' ? part : run(part.text, wrappers.map(tag => `<${tag}>`).join('') + part.open, part.close + wrappers.slice().reverse().map(tag => `</${tag}>`).join('')));
                }
                i = end + marker.length;
                continue;
            }
...
export function markdownToRichHtml(text) {
    ...
    const output = [];
    let paragraph = [];
    const inline = (value) => inlineRuns(value).map(renderRun).join('');
    const flush = () => {
        if (paragraph.length)
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (/^ {0,3}>/.test(line)) {
            ...
            i--;
            const alerts = { NOTE: '提示', TIP: '建议', IMPORTANT: '重要', WARNING: '警告', CAUTION: '注意' };
            const alert = /^\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]$/.exec(quoted[0]);
            if (alert)
                quoted[0] = `**${alerts[alert[1]]}**`;
        ...
        if (item) {
            ...
            const renderList = (start, depth) => {
                ...
                while (end < lines.length) {
                    ...
                    if (!next || next[1].length < indent || !!next[3] !== ordered)
                        break;
