...
export class TelegramProgress {
    ...
    fallbackMessage;
    mode;
    constructor(client, chatId, signal, enqueue, track, warn, maxLength = 4096) {
        this.client = client;
        this.chatId = chatId;
    ...
    finalHtml(text) {
        if (this.mode !== 'rich' || this.client.sendRichMessage === undefined || text.length > 24000)
            return undefined;
    ...
    async send() {
        ...
        if (this.mode === 'rich' && this.client.sendRichMessageDraft !== undefined) {
            await this.client.sendRichMessageDraft(this.chatId, this.draftId, `<tg-thinking>${escapeHtml(status)}</tg-thinking>${this.details()}${this.text ? `<p>${escapeHtml(this.text).replace(/\n/g, '<br>')}</p>` : ''}`, this.signal);
            return;
        }
        ...
        if (this.mode === 'draft' && this.client.sendMessageDraft !== undefined) {
            ...
            if (this.fallbackMessage === undefined) {
                ...
                try {
                    ...
                    if (/message to edit not found|message can't be edited/i.test(String(error)))
