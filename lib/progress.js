...
export class TelegramProgress {
    ...
    terminalNotice(reason) {
        if (this.stoppedByUser)
            return undefined;
        if (reason === 'completed')
            return this.paused && this.transcript.answer.trim() ? undefined : '✅ **任务已完成**';
    ...
    async send() {
        ...
        await this.client.sendRichMessageDraft(this.chatId, this.draftId, `<tg-thinking>${lines.map(escapeHtml).join('<br>')}</tg-thinking>\n\n${preview}`, this.signal);
    }
}
