...
export class TelegramResultCache {
    ...
    async deliver(chatId, markdown, send) {
        return this.exclusive(chatId, async () => {
            ...
            for (const text of typeof markdown === 'string' ? [markdown] : markdown)
                await send(text);
        });
    }
        ...
        return this.exclusive(chatId, async () => {
            ...
            if (typeof value !== 'object' || value === null || !('markdown' in value)
                || !(typeof value.markdown === 'string' || (Array.isArray(value.markdown)
                    && value.markdown.every(text => typeof text === 'string'))))
                throw new Error('Invalid Telegram result cache');
            const messages = typeof value.markdown === 'string' ? [value.markdown] : value.markdown;
