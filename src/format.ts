...
function markdownSegments(text: string): MarkdownSegment[] {
  ...
  let kind: MarkdownSegment['kind'] = 'plain'
  let buffer = ''
  ...
  if (kind === 'code') return [{ kind: 'plain', text }]
  if (buffer !== '') segments.push({ kind, text: buffer })
  return segments
}
...
export function markdownToHtmlChunks(text: string, maxLength: number): TelegramMessageChunk[] {
  ...
  let current: TelegramMessageChunk = { html: '', plain: '' }
  const flush = (): void => {
    if (current.plain === '' && current.html === '') return
    chunks.push(current)
    current = { html: '', plain: '' }
  }
