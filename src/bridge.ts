...
export class TelegramBridge {
  ...
  private async repairTelegramMembership(headers: readonly SessionHeader[]): Promise<void> {
    const accounted = new Set(
      this.ctx.workspaceRegistry.list().flatMap(workspace => workspace.sessionIds.map(String)),
    )
    for (const header of headers) {
      if (!String(header.id).startsWith('telegram:') || accounted.has(String(header.id))
        || header.cwd === undefined) continue
    ...
    catalog.forEach((entry, workspaceIndex) => {
      ...
      if (entry.sessions.length === 0) {
        ...
        entry.sessions.forEach((session, sessionIndex) => {
          ...
          if (title !== String(session.id)) lines.push(`　└ ${session.id}`)
        })
      }
      lines.push('')
    })
      ...
      try {
        ...
        for (const messageId of batch) {
          try {
            ...
            if (!isMissingMessage(error)) firstFailure ??= error
          }
        }
        if (firstFailure !== undefined) {
          this.ctx.logger.warn(
            '[telegram] turn cleanup failed after batch error (%s): %s',
            messageOf(batchError),
            messageOf(firstFailure),
          )
        }
      }
    }
