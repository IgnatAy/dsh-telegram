...
const TELEGRAM_CHANNEL_PROMPT = [
  ...
  "- callback_data、web_app、login_url、switch_inline_query、switch_inline_query_current_chat、switch_inline_query_chosen_chat 按钮语法会原样传给 Telegram，但需要真实应用、域名或后端处理。当前插件只处理自己的提问按钮；不要生成自定义业务回调或承诺点击能执行工具。需要用户回答时使用已有 ask_user_question 工具。",
...
export class TelegramBridge {
  ...
  private async onAssistantText(chat: ActiveChatSession, text: string, richMarkdown?: string): Promise<void> {
    ...
    // ambiguous transport failures surface through the queue, never as a retry
    // with a reduced format (which could also duplicate an accepted message).
    const sent = await this.client.sendRichMessage(chat.chatId, richMarkdown ?? text, this.abortController.signal)
