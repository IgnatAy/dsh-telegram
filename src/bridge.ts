...
const HELP_TEXT = [
  ...
  '',
  '**控制面板**',
  '/menu — 查看当前状态，选择工作区、会话、模型和推理强度；新建或删除会话。',
  '/new — 在当前工作区新建会话。',
  '/clear — 永久删除当前会话。',
  '',
  '**收集与排队**',
...
const MY_COMMANDS = [
  { command: 'menu', description: '打开控制面板：状态、工作区、会话和模型' },
...
export class TelegramBridge {
  ...
  private async handleMenuCallback(callback: TelegramCallbackQuery): Promise<void> {
    ...
    try {
      await this.client.editMessageReplyMarkup(chatId, menu.messageId, { inline_keyboard: [] }, this.abortController.signal)
    } catch (error) {
      this.ctx.logger.warn('[telegram] menu keyboard cleanup failed: %s', messageOf(error))
    }
    try {
      if (menu.fingerprint !== this.menuFingerprint(chatId)) {
        await this.showMenu(chatId, 'home', 0, undefined, '选择已变化，已刷新面板，请重新操作。')
        return
      }
      await action()
    } catch (error) {
      await this.showMenu(chatId, 'home', 0, undefined, `操作失败：${messageOf(error)}`)
    }
  }
  ...
  private async showMenu(
    ...
    try {
      ...
      const navigate = (next: typeof view, nextPage = 0, id?: string): Promise<void> => this.showMenu(chatId, next, nextPage, id)
      const lines = [
        '# 🎛 控制面板',
        this.formatSelectionSummary(state, info, catalog),
        `**运行状态**　${state.active?.agent.status === 'running' ? '运行中' : state.active ? '空闲' : '未连接会话'}`,
      ...
      if (notice) lines.push('', richLiteral(notice))
      const idle = (): void => {
      ...
      if (view === 'home') {
        lines.push('', '使用下方按钮查看完整列表并操作；列表按页显示。')
        ...
        paginate(catalog, entry => {
          lines.push(`**${richLiteral(entry.workspace.title)}** · ${entry.sessions.length} 个会话`, richLiteral(entry.workspace.path))
        ...
        paginate(entry.sessions, session => {
          ...
          lines.push(`${richLiteral(label)}${badges}`, richLiteral(String(session.id)))
        ...
        paginate(models, model => {
          ...
          lines.push(`${richLiteral(`${model.providerName} / ${model.name}`)}${selected ? ' · 当前' : ''}${model.inputModalities?.includes('image') ? ' · 支持图片' : ''}`)
          rows.push([button(`${selected ? '✓ ' : ''}${model.providerName} / ${model.name}`, async () => {
            idle()
        ...
        const choices = this.reasoningChoices(await this.resolveCurrentModel(state))
        lines.push('', '## 🧠 可用推理强度')
        ...
        paginate(choices, choice => {
          ...
          rows.push([button(`${state.preferences.selection.reasoningEffort === choice.effort ? '✓ ' : ''}${choice.name}`, async () => {
            idle()
      ...
      const sent = await this.client.sendRichMessage(chatId, lines.join('\n\n'), this.abortController.signal, { inline_keyboard: rows })
  ...
  private formatSelectionSummary(
    ...
    return [
      ...
      ...(state.workspace === undefined ? [] : [`└ ${richLiteral(state.workspace.path)}`]),
      `💬 **会话**　${richLiteral(session)}`,
