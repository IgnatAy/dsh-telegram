...
interface ActiveChatSession {
  ...
  /** Owning workspace selected through `/use`. */
...
const TELEGRAM_CHANNEL_PROMPT = [
  ...
  "- 可以使用 Markdown 标题、粗体、斜体、删除线、行内代码和围栏代码块。长回复用一个简短主标题和少量二、三级标题；段落之间只留一个空行，避免连续空行和复杂样式嵌套。",
  ...
  "- 比较数据时可使用标准 Markdown 表格，插件会渲染为原生表格。手机上优先 2–3 列、简短表头和短单元格；长解释放在表格外。不要用空格、竖线或框线字符手工画网格，不要把表格放进代码块。普通消息回退会改为“字段：值”列表。",
  ...
  "- 分隔线使用独立一行的 `---`，插件会渲染为原生分隔线；只在主要章节之间少量使用。避免靠空格对齐正文，避免超长连续字符。",
...
export class TelegramBridge {
  ...
  private async handleUpdate(update: TelegramUpdate): Promise<void> {
    ...
    if (stopped !== undefined) {
      // Native Stop is a chat-level request, just like /stop. Preview state
    ...
    switch (command) {
      ...
      case '/model': {
        ...
        try {
          ...
          if (selected === undefined) throw new Error(`模型编号 ${args[0]} 不存在，请重新发送 /model`)
      ...
      case '/stop':
        await this.stopCurrentTask(chatId)
        break
      case '/collect': {
        const active = this.chats.get(String(chatId))?.active
        if (active === undefined) {
      ...
      case '/reasoning': {
        ...
        try {
          ...
          if (selected === undefined) throw new Error(`思考强度 ${args[0]} 不存在，请重新发送 /reasoning`)
  ...
  private formatModelCatalog(catalog: readonly CatalogModel[], state: ChatState): string {
    if (catalog.length === 0) return '🤖 **可用模型**\n\n当前没有可用模型。'
    const current = state.preferences.selection
  ...
  private formatSelectionSummary(
    ...
    if (state.active !== undefined && state.active.sessionTitle === undefined && workspaces !== undefined) {
      ...
      if (title !== undefined && title !== '') session = `${title}（${state.active.sessionId}）`
    }
    return [
      '🎛 **当前选择**',
      '',
  ...
  private formatReasoningChoices(info: LlmResolvedModelInfo, state: ChatState): string {
    const choices = this.reasoningChoices(info)
    if (choices.length === 0) {
  ...
  private formatCatalog(catalog: readonly CatalogWorkspace[], state: ChatState): string {
    if (catalog.length === 0) return '🗂 **工作区与会话**\n\n目前没有已登记的工作区。请先在 Web UI 中添加工作区。'
    const lines: string[] = ['🗂 **工作区与会话**', '']
  ...
  private async useCatalogSelection(
    ...
    if (entry === undefined) throw new Error(`工作区编号 ${workspaceNumber} 不存在，请重新发送 /use`)
    if (sessionNumber === 0) {
  ...
  private async deliver(chatId: number, text: string): Promise<number[]> {
    ...
    try {
      ...
      if (isHtmlParseError(error)) {
        try {
          ...
          if (!this.stopped) this.ctx.logger.error('[telegram] delivery failed: %s', messageOf(fallbackError))
        }
      } else {
