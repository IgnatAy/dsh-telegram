...
最终结果的折叠记录来自本轮已确认的 `assistant/message`、`tool/call` 和 `tool/result` 事件，按事件顺序保存思考、中间正文、工具参数及文本结果。最后一条回复正文保留在折叠区外。失败重试的流式草稿不进入记录；非文本工具结果只显示类型标记，不上传附件。记录内容按文字转义，避免其中的 HTML 或未闭合代码块破坏最终答案。草稿仍仅显示简短状态和最近工具摘要。折叠内容同样计入 Telegram 富消息长度限制，超限沿用发送失败日志，不静默截断。
